...
export function IngredientList({ ingredients }: IngredientListProps) {
  const { unit } = useUnit();
  const [convertedIngredients, setConvertedIngredients] = useState<string | null>(null);
  ...
  const ingredientsToShow = unit === 'metric' && convertedIngredients ? convertedIngredients : ingredients;
    ...
    <div>
      <div className="flex items-center justify-between">
        ...
        {unit === 'metric' && convertedIngredients && (
            <Button variant="ghost" size="sm" onClick={() => setConvertedIngredients(null)}>
      ...
      {showConvertButton && (
        <div className="mb-4">
          <p className="text-sm text-muted-foreground mb-2">Your preference is set to Metric. Convert this recipe's ingredients?</p>
