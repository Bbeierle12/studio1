...
export function RecipeImportDialog({
  ...
  const [importMethod, setImportMethod] = useState<'url' | 'paste' | 'ai'>('url');
  const [url, setUrl] = useState('');
  const [html, setHtml] = useState('');
  const [aiText, setAiText] = useState('');
  const [hasProcessedShare, setHasProcessedShare] = useState(false);
  ...
  const parseAiMutation = useMutation({
    mutationFn: async (data: { text: string }) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to parse recipe with AI');
      }
    ...
    onSuccess: data => {
      setParsedRecipe(data.recipe);
      setEditedRecipe(data.recipe);
      toast.success('Recipe parsed successfully with AI!');
    },
    onError: (error: Error) => {
      toast.error(error.message);
    },
  ...
  const handleParse = () => {
    ...
    } else if (importMethod === 'ai' && aiText) {
      parseAiMutation.mutate({ text: aiText });
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-w-3xl max-h-[90vh]'>
        <DialogHeader>
          ...
          <DialogDescription>
            Import recipes from your favorite cooking websites or paste HTML
            content
          </DialogDescription>
        </DialogHeader>
        ...
        {!parsedRecipe ? (
          <>
            <Tabs
              value={importMethod}
              onValueChange={v => setImportMethod(v as 'url' | 'paste' | 'ai')}
            >
              ...
              <TabsContent value='paste' className='space-y-4'>
                <div className='space-y-2'>
                  ...
                  />
                  <p className='text-sm text-muted-foreground'>
                    Right-click on the recipe page → View Page Source → Copy and
                    paste here
                  </p>
                </div>
              </TabsContent>
              ...
              <TabsContent value='ai' className='space-y-4'>
                <div className='space-y-2'>
                  <Label htmlFor='aiText'>Social Media Post or Text</Label>
                  <Textarea
                    id='aiText'
                    placeholder='Paste a messy recipe from Instagram, a blog post without a recipe card, or an old text message here...'
                    value={aiText}
                    onChange={e => setAiText(e.target.value)}
                    className='min-h-[200px] text-sm font-mono'
                  />
                  <p className='text-sm text-muted-foreground'>
                    Our free AI will read this text and extract the ingredients, instructions, and details automatically!
                  </p>
                </div>
              </TabsContent>
            </Tabs>
            ...
            <DialogFooter>
              <Button
                onClick={handleParse}
                disabled={
                  parseMutation.isPending || parseAiMutation.isPending ||
                  (importMethod === 'url' && !url) ||
                  (importMethod === 'paste' && !html) ||
                  (importMethod === 'ai' && !aiText)
                }
              >
                {parseMutation.isPending || parseAiMutation.isPending ? (
                  <>
            ...
            <div className='space-y-4'>
              ...
              <Card>
                ...
                <CardContent className='space-y-4'>
                  ...
                  <div className='space-y-2'>
                    <Label>
                      Ingredients ({editedRecipe?.ingredients?.length || 0})
                    </Label>
                  ...
                  <div className='space-y-2'>
                    <Label>
                      Instructions ({editedRecipe?.instructions?.length || 0}{' '}
                      steps)
                    </Label>
                    <ScrollArea className='h-[150px] rounded-md border p-3'>
